...
interface GenerateCommands {
  appName: string
  cwd: string
}

export function generateCommands({ appName, cwd }: GenerateCommands) {
  return {
    language: {
      en: {
    ...
    validateExist: {
      en: {
        exist: `🗑  The old ${logger.greenBright.raw(appName)} folder was deleted successfully.`,
        beforeDel: `${logger.bgBlue.raw('INFO')} Ready to delete...`,
        afterDel: 'Delete done.',
        newly: `🎉 The new ${logger.greenBright.raw(appName)} folder was created successfully.\n`
      },
      cn: {
        exist: `🗑  旧目录 ${logger.greenBright.raw(appName)} 已经被删除.`,
        beforeDel: `${logger.bgBlue.raw('INFO')} 准备执行删除...`,
        afterDel: '删除完毕.',
        newly: `🎉 新目录 ${logger.greenBright.raw(appName)} 已经被创建.\n`
      }
    },
    installDep: {
      en: {
        created: `✨  Creating project in ${logger.yellow.raw(pathResolve(cwd, appName))}.`,
        start: 'Installing project dependencies. This might take a while...',
        end: `🎉  Successfully created project dependencies for ${logger.yellow.raw(appName)}.`
      },
      cn: {
        created: `✨  项目创建在 ${logger.yellow.raw(pathResolve(cwd, appName))}.`,
        start: '正在安装依赖. 这需要花费一点时间...',
        end: `🎉  ${logger.yellow.raw(appName)} 项目依赖安装成功.`
      }
    },
    manager: {
      en: {
    ...
    npmMirror: {
      en: {
        message:
          'Do you need to install Taobao Mirror to speed up the installation of dependencies?'
      },
      cn: {
